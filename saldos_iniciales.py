...
def run():
    # === CREAR PERÍODO CONTABLE (NO SE CIERRA AUTOMÁTICAMENTE) ===
    periodo, _ = PeriodoContable.objects.get_or_create(
        nombre="Noviembre 2025",
        inicio=date(2025, 11, 1),
        fin=date(2025, 11, 30),
    ...
    # === CALCULAR SALDOS FINALES DEL PERÍODO ===
    for cuenta in Cuenta.objects.filter(es_imputable=True):
        movs = DetalleAsiento.objects.filter(asiento__periodo=periodo, cuenta=cuenta)\
            .aggregate(debe_total=Sum("debe"), haber_total=Sum("haber"))
        debe = movs["debe_total"] or Decimal("0.00")
        ...
        # determinar normalidad por tipo (usa tus códigos reales)
        if cuenta.tipo_cuenta in ["ACT", "GAS", "COS"]:
            saldo = debe - haber
            saldo_deudor = max(Decimal("0.00"), saldo)
            saldo_acreedor = max(Decimal("0.00"), -saldo)
    ...
    print("📊 Saldos calculados correctamente (sin cerrar período).")

    # Nota: NO se realizan asientos de cierre, no se marca el período como cerrado
    # y NO se crea ni abre un nuevo período. Si quieres que el script haga alguna
    # acción adicional (por ejemplo exportar balances o listar cuentas con déficit),
    # dime cuál y la agrego.
